...
* extension[estimatedDischargeDate] ^min = 0
* identifier 1..1
...
* identifier.system ^example[0].label = "OID de l'ASIP de gestion des organisations (1.2.250.1.71.4.2.2) suivi du FINESS suivi de la branche z qui gère les id de venue/mouvements si l'établissement n'a pas d'OID (sinon utilisation de l'OID de l'établissement)"
...
* hospitalization.preAdmissionIdentifier.use from IdentifierUse (required)
* hospitalization.preAdmissionIdentifier.use ^binding.extension[0].url = "http://hl7.org/fhir/StructureDefinition/elementdefinition-bindingName"
* hospitalization.preAdmissionIdentifier.use ^binding.extension[=].valueString = "IdentifierUse"
...
* location.status from EncounterLocationStatus (required)
